...
class TestBankOnAntiAlucinacao:
    ...
    def test_termos_fora_do_escopo_devem_retornar_mensagem_padrao(self, pergunta_fora_escopo):
            """Garante que qualquer termo ausente na base retorne a mensagem de recusa."""
            resposta = responder_pergunta(pergunta_fora_escopo)
            
            # Opção A: Se você quer validar a presença do trecho chave de recusa
            trecho_chave_recusa = "não consta no nosso guia de nivelamento"
            
            # Opção B: Caso você queira validar a frase de recusa inteira
            assert MENSAGEM_RECUSA_ESPERADA in resposta.strip() or trecho_chave_recusa in resposta, (
                f"Falha para a pergunta: '{pergunta_fora_escopo}'.\n"
                f"Esperado conter: {MENSAGEM_RECUSA_ESPERADA}\n"
                f"Recebido: {resposta}"
            )
